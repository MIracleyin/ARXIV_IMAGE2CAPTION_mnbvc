...
def latex_to_image(latex_code, output_image_path, dpi=300, tmp_res_dir=None):
    if tmp_res_dir is None:
        ...
        tmp_res_dir = os.path.join(tmp_res_dir, 'tmp_' + str(time.time()))
        os.makedirs(tmp_res_dir, exist_ok=True)
    ...
    temp_tex = "temp.tex"
    with open(os.path.join(tmp_res_dir, temp_tex), "w") as f:
        f.write(
            r"""
...
"""
    ...
    try:
        ...
        assert os.path.exists(os.path.join(tmp_res_dir, pdf_path)), "convert latex2 pdf failded, no pdf generated"
        ...
        if output_image_path.lower().endswith(".pdf"):
            ...
            images = convert_from_path(os.path.join(tmp_res_dir, corp_pdf_path), dpi=dpi)
            if images:
...
"""
    ...
    save_dir = "./resource_tmp/table2img_test"
    os.makedirs(save_dir, exist_ok=True)

    for idx, each_data in enumerate(load_jsonl('resource_tmp/total_json.jsonl')[:3]):
        latex_to_image(each_data['table/equation'].replace('\n\n', '\n'), f"{save_dir}/{idx}_output.png", dpi=300)
        print("LaTeX content rendered and saved as output.png")
